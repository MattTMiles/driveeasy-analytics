...
from pydea.annotations import _handle_meas_date, Annotations
#
...
class Raw:
    def __init__(self, filename=None, use_pandas=True, to_wav_data=False, interpolate_time_index=True, time_shift_to_UTC=None):
        self.data_block = []
    ...
    def set_annotations(self, annotations, emit_warnings=True):
    ...
    def append_block(self, fbgs_block):
        if isinstance(fbgs_block, Raw):
            self.data_block.extend(fbgs_block.data_block)
            logger.info('FBGS data block attached.')
        else:
            logger.warning('fbgs_block is not an instance of Raw type.')
