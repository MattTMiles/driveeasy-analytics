...
df_ch3 = pd.read_pickle('wav_video2-9_18AM-9_23AM_ch_1.pickle')
...
df_ch4 = pd.read_pickle('wav_video2-9_18AM-9_23AM_ch_2.pickle')
...
for i in range(int(lane_left1.shape[0]/window_size)-1):
    ...
    if window_i1[window_i1.abs()>0.001].any().any():
        ...
        if sensor1 | sensor2 >4:
            ...
            if sensor1==sensor2:
                time_peaks1, peak_prop1 = signal.find_peaks(abs(window_i1.iloc[:,sensor1]), distance=10, height = 0.001, width=0.5)
                time_peaks2, peak_prop2 = signal.find_peaks(abs(window_i2.iloc[:,sensor2]), distance=10, height = 0.001, width=0.5)
                ...
                if n_peaks > 1:  
                    ...
                    for j in range(10):
                        ...
                        window_s1 = lane_left1.iloc[window_s_start:window_s_end, :] - lane_left1.iloc[window_s_start, :].values.squeeze() # zero the window
                        window_s2 = lane_left2.iloc[window_s_start:window_s_end, :] - lane_left2.iloc[window_s_start, :].values.squeeze() # zero the window
                        
                        # get the peaks in this sliding windows
                        time_peaks, peak_prop = signal.find_peaks(abs(window_s1.iloc[:,sensor1]), distance=10, height = 0.001, width=0.5)
                    ...
                    if len(neighbors_idx) >= 2:
                        ...
                        t_start = str(lane_left1.index[window_size*(i)])
                        t_start = datetime.strptime(t_start, '%Y-%m-%d %H:%M:%S.%f')
                        t_start_str = datetime.strftime(t_start, '%Y_%m%d_%H_%M_%S')
                        
                        # wavelength shift
                        wav1 = lane_left1.iloc[event_start:event_end,:]
                        wav2 = lane_left2.iloc[event_start:event_end,:]
                        
                        speed = -speed_estimate(wav1, wav2)
                        speed_dic.append([event_start, speed])    
                        plot_heatmap(event_window, 'Lane23_Event'+ str(count), t_start_str +', speed (km/h)'+str(speed))
                        
                        # save the events                        
                        save_events(count, t_start_str, wav1, wav2, 3, 4, sensor1, sensor2)
                        ...
                        # wavelength shift
                        wav1 = lane_left1.iloc[window_size*(i):window_size*(i+1),:]
                        wav2 = lane_left2.iloc[window_size*(i):window_size*(i+1),:]
                        ...
                        # visualization                        
                        plot_heatmap(lane_left1.iloc[window_size*(i):window_size*(i+1),:], 'Lane23_Event'+ str(count), 
                                     t_start_str +', '+"%.2f" %speed+'km/h')
